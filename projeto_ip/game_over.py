...
def exibir_game_over(screen, clock):
    game_over = True
    ...
    try:
        ...
        img_enter = pg.image.load(caminho_enter).convert_alpha()
        img_enter = pg.transform.scale(img_enter, (400, 200))
    except Exception as e:
        print(f"Erro ao carregar imagem do ENTER: {e}")
        img_enter = font_ESC.render("Pressione ESC para sair", True, (50, 50, 50))

    try:
        caminho_bg = os.path.join(DIRETORIO_BASE, "assets", "backgrounds", "game_over_bg.png")
    ...
    try:
        caminho_musica = os.path.join(DIRETORIO_BASE, "assets", "músicas", "game_over.mp3")
        pg.mixer.music.load(caminho_musica)
        pg.mixer.music.set_volume(0.5)
        pg.mixer.music.play(-1)
    except Exception as e:
        print(f"Erro na músicsa: {e}")
